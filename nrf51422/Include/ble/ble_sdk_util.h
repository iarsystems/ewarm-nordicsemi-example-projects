...
#define CRITICAL_REGION_ENTER()                                                             \
    {                                                                                       \
        ...
        if (CURRENT_INT_PRI != NRF_APP_PRIORITY_HIGH)                                       \
        {                                                                                   \
...
#define CRITICAL_REGION_EXIT()                                                              \
        if (CURRENT_INT_PRI != NRF_APP_PRIORITY_HIGH)                                       \
        {                                                                                   \
            uint32_t ERR_CODE = nrf_nvic_critical_region_exit(IS_NESTED_CRITICAL_REGION);   \
            if (ERR_CODE != NRF_SUCCESS)                                                    \
            {                                                                               \
                ASSERT(false);                                                              \
            }                                                                               \
 ...
 * @retval   NRF_APP_PRIORITY_HIGH    We are running in Application High interrupt level.
 * @retval   NRF_APP_PRIORITY_LOW     We are running in Application Low interrupt level.
 * @retval   NRF_APP_PRIORITY_THREAD  We are running in Thread Mode.
 */
