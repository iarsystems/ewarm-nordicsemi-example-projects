...
enum
{
  SVC_BLE_GATTS_DB_LOAD  = BLE_GATTS_SVC_BASE,    /**< Load an attribute database. */
  SVC_BLE_GATTS_SERVICE_ADD,                      /**< Add a service. */
  SVC_BLE_GATTS_INCLUDE_ADD,                      /**< Add an included service. */
  SVC_BLE_GATTS_CHARACTERISTIC_ADD,               /**< Add a characteristic. */
  SVC_BLE_GATTS_DESCRIPTOR_ADD,                   /**< Add a generic attribute. */
  SVC_BLE_GATTS_MD_SET,                           /**< Set attribute metadata. */
  SVC_BLE_GATTS_MD_GET,                           /**< Get attribute metadata. */
  SVC_BLE_GATTS_VALUE_SET,                        /**< Set an attribute value. */
  SVC_BLE_GATTS_VALUE_GET,                        /**< Get an attribute value. */
  SVC_BLE_GATTS_HVX,                              /**< Handle Value Notification or Indication. */
  SVC_BLE_GATTS_SERVICE_CHANGED,                  /**< Perform a Service Changed Indication to one or more peers. */
  SVC_BLE_GATTS_RW_AUTHORIZE_REPLY,               /**< Reply to an authorization request for a read or write operation on one or more attributes. */ 
  SVC_BLE_GATTS_SYS_ATTR_SET,                     /**< Set the persistent system attributes for a connection */  
  SVC_BLE_GATTS_SYS_ATTR_GET,                     /**< Get updated persistent system attributes after terminating a connection */
};
...
typedef struct
{
  ble_gatts_char_properties_t char_properties;      /**< Characteristic Properties. */
...
enum
{
  BLE_GATTS_EVT_MTU_UPDATE  = BLE_GATTS_EVT_BASE, /**< MTU Update. */
  BLE_GATTS_EVT_WRITE,                            /**< Write operation performed. */
  BLE_GATTS_EVT_RW_AUTHORIZE_REQUEST,             /**< Read/Write Authorization request. */
  BLE_GATTS_EVT_SYS_ATTR_MISSING,                 /**< A persistent system attribute access is pending, awaiting a ble_gatts_sys_attr_set(). */
...
/**@brief Event structure for BLE_GATTS_EVT_MTU_UPDATE. */
typedef struct
{
  uint16_t          att_mtu;                      /**< Negotiated MTU. */
} ble_gatts_evt_mtu_update_t;
...
typedef struct
{
  ...
  union
  {
    ble_gatts_evt_mtu_update_t            mtu_update;         /**< MTU Update Event Parameters. */
...
/**@brief Load a preconfigured GATT attribute database.
 * @note Not yet implemented.
 *
 * @param[in] len    Length in bytes of the buffer data.
 * @param[in] p_data Pointer to the beginning of the buffer containing the raw database data.
 * @return @ref NRF_ERROR_NOT_SUPPORTED
 *
 */
SVCALL(SVC_BLE_GATTS_DB_LOAD, uint32_t, ble_gatts_db_load(uint32_t len, uint8_t const* const p_data));


/**@brief Add a service declaration to the local server database.
 *
 * @note The Service Changed characteristic is currently not implemented. Thus, to conform to the 
 *       Blutetooth specifications, the application must not add, modify or remove services, 
 *       characteristics or descriptors while the device is connected or bonded to another device.
 *
 ...
 */
SVCALL(SVC_BLE_GATTS_SERVICE_ADD, uint32_t, ble_gatts_service_add(uint8_t type, ble_uuid_t const*const p_uuid, uint16_t *const p_handle));
 ...
 */
SVCALL(SVC_BLE_GATTS_INCLUDE_ADD, uint32_t, ble_gatts_include_add(uint16_t service_handle, uint16_t inc_srvc_handle, uint16_t *const p_include_handle));
 ...
 */
SVCALL(SVC_BLE_GATTS_CHARACTERISTIC_ADD, uint32_t, ble_gatts_characteristic_add(uint16_t service_handle, ble_gatts_char_md_t const*const p_char_md, ble_gatts_attr_t const*const p_attr_char_value, ble_gatts_char_handles_t *const p_handles));
 ...
 * @return @ref NRF_ERROR_DATA_SIZE Invalid data size(s) supplied.
 */
SVCALL(SVC_BLE_GATTS_DESCRIPTOR_ADD, uint32_t, ble_gatts_descriptor_add(uint16_t char_handle, ble_gatts_attr_t const * const p_attr, uint16_t* const p_handle));

/**@brief Set the metadata for a given attribute.
 *
 * @param[in] handle    Attribute handle.
 * @param[in] p_attr_md Pointer to the desired attribute metadata.
 *
 * @return @ref NRF_SUCCESS Success.
 * @return @ref NRF_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @return @ref NRF_ERROR_INVALID_PARAM Invalid parameter(s) supplied.
 * @return @ref NRF_ERROR_NOT_FOUND Attribute not found.
 */
SVCALL(SVC_BLE_GATTS_MD_SET, uint32_t, ble_gatts_md_set(uint16_t handle, ble_gatts_attr_md_t const * const p_attr_md));

/**@brief Get the metadata for a given attribute.
 *
 * @param[in] handle     Attribute handle.
 * @param[out] p_attr_md Pointer to the structure to be filled in with data.
 *
 * @return @ref NRF_SUCCESS Success.
 * @return @ref NRF_ERROR_INVALID_ADDR Invalid pointer supplied.
 * @return @ref NRF_ERROR_NOT_FOUND Attribute not found.
 */
SVCALL(SVC_BLE_GATTS_MD_GET, uint32_t, ble_gatts_md_get(uint16_t handle, ble_gatts_attr_md_t* const p_attr_md));
 ...
 * @return @ref NRF_ERROR_DATA_SIZE Invalid data size(s) supplied.
 */
SVCALL(SVC_BLE_GATTS_VALUE_SET, uint32_t, ble_gatts_value_set(uint16_t handle, uint16_t offset, uint16_t* const p_len, uint8_t const * const p_value));
 ...
 * @return @ref NRF_ERROR_NOT_FOUND Attribute not found.
 */
SVCALL(SVC_BLE_GATTS_VALUE_GET, uint32_t, ble_gatts_value_get(uint16_t handle, uint16_t offset, uint16_t *const p_len, uint8_t* const p_data));
 ...
 */
SVCALL(SVC_BLE_GATTS_HVX, uint32_t, ble_gatts_hvx(uint16_t conn_handle, ble_gatts_hvx_params_t const*const p_hvx_params));
 ...
 *          be issued.
 *
 * @note    Some of the restrictions and limitations that apply to @ref ble_gatts_hvx also apply here.
 *
 * @param[in] conn_handle  Connection handle.
 ...
 * @return @ref BLE_ERROR_GATTS_SYS_ATTR_MISSING System attributes missing.
 */
SVCALL(SVC_BLE_GATTS_SERVICE_CHANGED, uint32_t, ble_gatts_service_changed(uint16_t conn_handle, uint16_t start_handle, uint16_t end_handle));
 ...
 */
SVCALL(SVC_BLE_GATTS_RW_AUTHORIZE_REPLY, uint32_t, ble_gatts_rw_authorize_reply(uint16_t conn_handle, ble_gatts_rw_authorize_reply_params_t const*const p_rw_authorize_reply_params));
 ...
 * @return @ref NRF_ERROR_NO_MEM Not enough memory to complete operation.
 */ 
SVCALL(SVC_BLE_GATTS_SYS_ATTR_SET, uint32_t, ble_gatts_sys_attr_set(uint16_t conn_handle, uint8_t const*const p_sys_attr_data, uint16_t len)); 
 ...
 *          should be restored using ble_gatts_sys_attr_set().
 ...
 * @return @ref NRF_ERROR_DATA_SIZE Invalid data size(s) supplied.
 */ 
SVCALL(SVC_BLE_GATTS_SYS_ATTR_GET, uint32_t, ble_gatts_sys_attr_get(uint16_t conn_handle, uint8_t * const p_sys_attr_data, uint16_t* const p_len)); 
