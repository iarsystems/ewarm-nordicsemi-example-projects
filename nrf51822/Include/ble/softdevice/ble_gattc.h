...
enum
{
  ...
  SVC_BLE_GATTC_PRIMARY_SERVICES_DISCOVER,             /**< Primary Service Discovery. */
  SVC_BLE_GATTC_RELATIONSHIPS_DISCOVER,                /**< Relationship Discovery. */
  SVC_BLE_GATTC_CHARACTERISTICS_DISCOVER,              /**< Characteristic Discovery. */
  SVC_BLE_GATTC_DESCRIPTORS_DISCOVER,                  /**< Characteristic Descriptor Discovery. */
  SVC_BLE_GATTC_CHAR_VALUE_BY_UUID_READ,               /**< Read Characteristic Value by UUID. */
  SVC_BLE_GATTC_READ,                                  /**< Generic read. */
  SVC_BLE_GATTC_CHAR_VALUES_READ,                      /**< Read multiple Characteristic Values. */
  SVC_BLE_GATTC_WRITE,                                 /**< Generic write. */
  SVC_BLE_GATTC_HV_CONFIRM                             /**< Handle Value Confirmation. */
};
...
/**@brief Start/end handle for an operation. */
typedef struct
{
...
enum
{
  ...
  BLE_GATTC_EVT_PRIM_SRVC_DISC_RSP,                  /**< Primary Service Discovery Response event.  */
...
/**@brief Event structure for BLE_GATTC_EVT_MTU_XCHG_RSP. */
typedef struct
{
  uint16_t att_mtu;                    /**< Negotiated MTU. */
} ble_gattc_evt_mtu_xchg_rsp_t;
...
typedef struct
{
  ...
  union
  {
    ble_gattc_evt_mtu_xchg_rsp_t                mtu_xchg_rsp;
 ...
 */
SVCALL(SVC_BLE_GATTC_MTU_EXCHANGE, uint32_t, ble_gattc_mtu_exchange(uint16_t conn_handle));
 ...
 */
SVCALL(SVC_BLE_GATTC_PRIMARY_SERVICES_DISCOVER, uint32_t, ble_gattc_primary_services_discover(uint16_t conn_handle, uint16_t start_handle, ble_uuid_t const * const p_srvc_uuid));
 ...
 * @return @ref NRF_ERROR_BUSY Client procedure already in progress
 */
SVCALL(SVC_BLE_GATTC_RELATIONSHIPS_DISCOVER, uint32_t, ble_gattc_relationships_discover(uint16_t conn_handle, ble_gattc_handle_range_t const * const p_handle_range));
 ...
 * @return @ref NRF_ERROR_BUSY Client procedure already in progress
 */
SVCALL(SVC_BLE_GATTC_CHARACTERISTICS_DISCOVER, uint32_t, ble_gattc_characteristics_discover(uint16_t conn_handle, ble_gattc_handle_range_t const * const p_handle_range));
 ...
 * @return @ref NRF_ERROR_BUSY Client procedure already in progress
 */
SVCALL(SVC_BLE_GATTC_DESCRIPTORS_DISCOVER, uint32_t, ble_gattc_descriptors_discover(uint16_t conn_handle, ble_gattc_handle_range_t const * const p_handle_range));
 ...
 * @return @ref NRF_ERROR_BUSY Client procedure already in progress
 */
SVCALL(SVC_BLE_GATTC_CHAR_VALUE_BY_UUID_READ, uint32_t, ble_gattc_char_value_by_uuid_read(uint16_t conn_handle, ble_uuid_t const * const p_uuid, ble_gattc_handle_range_t const * const p_handle_range));
 ...
 * @return @ref NRF_ERROR_BUSY Client procedure already in progress
 */
SVCALL(SVC_BLE_GATTC_READ, uint32_t, ble_gattc_read(uint16_t conn_handle, uint16_t handle, uint16_t offset));
 ...
 * @return @ref NRF_ERROR_BUSY Client procedure already in progress
 */
SVCALL(SVC_BLE_GATTC_CHAR_VALUES_READ, uint32_t, ble_gattc_char_values_read(uint16_t conn_handle, uint16_t const * const p_handles, uint16_t handle_count));
 ...
 */
SVCALL(SVC_BLE_GATTC_WRITE, uint32_t, ble_gattc_write(uint16_t conn_handle, ble_gattc_write_params_t const * const p_write_params));
 ...
 * @return @ref NRF_ERROR_BUSY Client procedure already in progress
 */
SVCALL(SVC_BLE_GATTC_HV_CONFIRM, uint32_t, ble_gattc_hv_confirm(uint16_t conn_handle, uint16_t handle));
